...
class Cell:
    ...
    def countAlive(self, bias, emptyCells = 0, aliveCount = 0): # emptyCells is set to 0 cuz if its set to set(), then python keeps it pointing to the same set everytime. (sometimes causes problems)(not in this case tho)
        if bias == 0:
            ...
            for neighbor in self.neighbors:
                ...
                elif self.next == 1: emptyCells.add(neighbor)
...
def decideState(board, bias, nextBoard = ''):
    emptyCells = set()
    if bias ==0:
        for cell in board.values():
            if cell.next == 1: nextBoard += 'x'
            else: nextBoard += ' '
            if cell.state == 1: # if cell is dead, ignore for now. we loop on en later but smartly (loop on the dead cells only if they neighbor the alive ones)
                aliveCount = cell.countAlive(bias, emptyCells)
        ...
        for cell in emptyCells: # loop on dead cells
            aliveCount = cell.countAlive(bias)
...
if __name__ == '__main__':
    ...
    #printBoard(
    loadStructure(board, offX, offY, structureName, randomness)#, cols, cellDead, cellAlive) # ('random', rarity), 'gilder', 
    start = time.time()
    bias = 0
    for generation in range(worldEnd+1): #for loop is faster
        tick = time.time()
        board, bias, newBoard = decideState(board, bias)
        print(newBoard)
